...
label titanDinner1DayKhd:
    ...
    show mkr norm sunset at left with Dissolve(0.5)             #Макар
    ...
    show mkr norm close sunset at left                          #Макар 
    ...
    if evCibClubJoin:
        ...
        mkr "это что, чифир?"
        th "не уж то я нашел здравомыслящего человека?"
        bgd "дааа"
        "Богдан опять похвастался своими изысканными вкусами"
        mkr "и ты им питаешься"
        bgd "именно, и никакой другой еды мне не надо"
        mkr "феноменально"
        bgd "не то слово"
    ...
    show mkr smile close sunset at left                         #Макар улыбается
    mkr "то есть не мы, а Ульяна, и не на склад, а так, по крышам прыгала"
    "Макар подмигнул Богдану и переглянулся с Тохой"
    ...
    if evCibClubJoin:
        ...
        "Тоха сразу повеселел, видно общения с людьми кроме Макара ему явно не хватало"
        ...
        show mkr norm close at right                            #Макар нормальный
